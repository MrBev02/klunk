...
import { extractPaper, toLines, type PageText } from './extract'
...
function shuffled(p: PageText): PageText {
 ...
 */
const MARK_COLUMN = 103

/** A line of question text with its marks in the margin. */
function marked(text: string, marks: number): string {
  return text.padEnd(MARK_COLUMN, ' ') + marks
}

/** The marks alone on a line, as happens when they centre against wrapped text. */
function markOnly(marks: number): string {
