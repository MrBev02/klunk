...
describe('an answer that lands', () => {
  ...
  it('says a model read it, on the question it marks', () => {
    const out = applyMarking([adopted(choiceQuestion('1'))], marking([{ number: 1, answers: ['C'] }]), CTX)
  ...
  it('answers a multiple-response question with every letter given', () => {
    ...
    const out = applyMarking([adopted(question)], marking([{ number: 10, answers: ['A', 'C'] }]), CTX)
...
describe('an answer that does not land', () => {
  ...
  it('reports an entry for a question that was never read', () => {
    const out = applyMarking([adopted(choiceQuestion('1'))], marking([{ number: 40, answers: ['A'] }]), CTX)
  ...
  it('says how much of the paper the reply covered', () => {
    ...
    expect(out.notes.join(' ')).toContain('1 of the 2 questions were marked, 1 of them with an answer')
  })
})
...
describe('the criteria', () => {
  ...
  it('puts a band on the question, top mark and all', () => {
    ...
    expect(out.adopted[0]!.question.markingGuide!.answersCouldInclude).toEqual(['Cost of retooling'])
  })
